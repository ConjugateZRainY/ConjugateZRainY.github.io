...
- When using the `hugo server` command locally on macOS, it defaults to the `http`protocol. You need to configure certificates; the specific method is to install `mkcert` using `brew`. The specific commands are as follows (remember to run the following commands in the blog directory):
...
- In local preview, Safari may not necessarily display the website icon correctly, while Chrome displays it perfectly. After actually deploying to GitHub Pages, the content viewed online is normal.
